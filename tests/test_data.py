...
class TestDataUtils(unittest.TestCase):
    ...
    def test_filter_transcripts(self):
        data = {
            "qv": [30, 10, 25],
            "feature_name": ["gene1", "NegControlProbe_gene2", "gene3"],
        }
    ...
    def test_compute_transcript_metrics(self):
        ...
        self.assertAlmostEqual(
            metrics["percent_non_assigned_cytoplasmic"], 100.0
        )
        self.assertEqual(len(metrics["gene_metrics"]), 2)
        self.assertTrue(
            "gene1" in metrics["gene_metrics"]["feature_name"].values
        )
        self.assertTrue(
            "gene2" in metrics["gene_metrics"]["feature_name"].values
        )
    ...
    def test_crop_transcripts(self):
        ...
        self.assertEqual(
            cropped_sample.transcripts_df.iloc[0]["feature_name"], "gene1"
        )
    ...
    def test_unassign_all_except_nucleus(self):
        ...
        self.assertEqual(
            unassigned_df.loc[
                unassigned_df["overlaps_nucleus"] == 0, "cell_id"
            ].values[0],
            "UNASSIGNED",
        )
    ...
    def test_build_tx_graph(self):
        ...
        data = Data(
            tx={
                "pos": torch.tensor([[0, 0], [1, 1], [2, 2]], dtype=torch.float)
            }
        )
        transformed_data = transform(data)
        self.assertTrue(
            "edge_index" in transformed_data["tx", "neighbors", "tx"]
        )
