...
def predict(
    ...
    use_cc: bool = True,
    knn_method: str = 'cuda',
    n_jobs: int = 2  # Number of parallel jobs
) -> pd.DataFrame:
    """
    ...
    delayed_batches = [delayed(predict_batch)(lit_segger, batch, score_cut, receptive_field, use_cc, knn_method)
                        for batch in data_loader]

        # Compute all tasks in parallel with progress bar
    with ProgressBar():
        all_assignments = delayed(delayed_batches).compute()

    # Convert the results to Dask DataFrames and combine them
    all_assignments_dd = [dd.from_pandas(assignments, npartitions=1) for assignments in all_assignments]
...
def segment(
    ...
    **anndata_kwargs
    ...
    # Get dataloaders for training, validation, and test
    train_dataloader = dm.train_dataloader()
    test_dataloader  = dm.test_dataloader()
    val_dataloader   = dm.val_dataloader()

    # Perform predictions for each dataset
    ...
    transcripts_df_filtered = transcripts_df.merge(seg_final_dd, on='transcript_id', how='outer')
    ...
    if use_cc:
        ...
        transcript_idx_map = pd.Series(transcripts_df_filtered.index, index=transcripts_df_filtered['transcript_id']).to_dict()
        ...
        n, comps = cc(scipy_coo_matrix((np.ones(len(source_indices)), 
                      ...
                      connection="weak", directed=False)
