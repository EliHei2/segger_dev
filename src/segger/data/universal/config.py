...
def check_extension(path: os.PathLike, allowed_exts: set[str]):
    """
...
def check_field_names(path: Path, names: List[str]):
    """
...
class DataConfig(BaseModel):
    ...
    See 'segger/data/universal/_configs/template.yaml' for more detail.
    """
    ...
    negative_sampling_ratio: PositiveFloat = Field(ge=1)
    ...
    def validate_parquets(self):
        ...
        check_extension(self.tx_path, {".parquet"})
        check_field_names(self.tx_path, tx_fields)
        check_extension(self.bd_path, {".parquet"})
        check_field_names(self.bd_path, ["geometry"])
        return self
