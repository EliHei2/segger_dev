...
PROJECT_NAME="project24_MNG_pqdm" # this is the folder tag for the datasets DATA_ROOT/PROJECT_NAME and the folder in MODELS_ROOT where the models are gonna be saved
...
for full_path in "$DATA_ROOT/$PROJECT_NAME"/*; do
    if [ -d "$full_path" ]; then
        ...
        bsub -o logs/train_${dataset_tag}.log \
             -gpu num=4:gmem=20.7G \
             -R "rusage[mem=100GB]" \
             -q gpu-debian \
             python path/to/train_batch.py \
