...
def main():
    ...
    # Get gene type to color
    gene_type_to_color = dict(zip(gene_types_dict.values(), gene_types_dict.keys()))
    ...
    # if clustering results are not found, run hierarchical clustering
    if not (Path('results') / f'clustering_results_tx-tx.pkl').exists():
        ...
        with open(Path('intermediate_data') / f'clustering_results_tx-tx.pkl', 'wb') as f:
            pickle.dump(clustering_results, f)
    else:
        # load clustering results
        with open(Path('intermediate_data') / f'clustering_results_tx-tx.pkl', 'rb') as f:
            clustering_results = pickle.load(f)
    
    # print the clustering results
    print(clustering_results['n_neighbors_5_n_components_2']['cluster_labels'])
